// src/profile_offsets.rs — updated for fold() 3-arg signature
...
fn main() {
    ...
    for path in &args {
        ...
        // Pass 0 for filter_flag — profiling runs on raw data, no pre-filter.
        ...
        let ob = ob_per_fold.first().copied().unwrap_or(mbfa::opcode::OFFSET_BITS_DEFAULT);
        let lb = lb_per_fold.first().copied().unwrap_or(mbfa::opcode::LENGTH_BITS_DEFAULT);

        let tokens = match mbfa::bitreader::read_tokens(&compressed, ob, lb) {
            Ok(t)  => t,
        ...
        let savings_pct = if total_br_bits > 0.0 { savings_bits / total_br_bits * 100.0 } else { 0.0 };

        println!(
            "{:<25}  {:>8}  {:>8}  {:>8}  {:>8}  {:>9.2}%  {:>9.2}%  {:>9.2}%   est saving ~{:.1}% of offset bits",
            name, total, hit1, hit2, hit3,
            pct(hit1), pct(hit2), pct(hit3), savings_pct
        );
    ...
    for token in tokens {
        if let mbfa::opcode::Token::Backref { offset, .. } = token {
            ...
            if recent.front() != Some(offset) {
                ...
                if recent.len() > 3 { recent.pop_back(); }
            }
        }
    }
